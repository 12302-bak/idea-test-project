...
/**
 * <dl>
 * <dt>PageInterceptor</dt>
 * <dd>Description:分页拦截器</dd>
 * </dl>
 */
@Intercepts(
	@Signature(
...
public class PageInterceptor implements Interceptor {
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		if (rowBounds == null || rowBounds == RowBounds.DEFAULT) {
			...
			// 计算分页
			page.setPageNumber(rowBounds.getOffset() / rowBounds.getLimit() + 1);
	...
	private String getPageSql(String sql, RowBounds rowBounds) {
		...
		if ("mysql".equals(dialect)) {
			pageSql.append(sql);
			pageSql.append(" limit " + rowBounds.getOffset() + ","
					+ rowBounds.getLimit());
		} else if ("hsqldb".equals(dialect)) {
			pageSql.append(sql);
			pageSql.append(" LIMIT " + rowBounds.getOffset() + " OFFSET "
					+ rowBounds.getLimit());
		} else if ("oracle".equals(dialect)) {
			pageSql.append("select * from ( select temp.*, rownum row_id from ( ");
			pageSql.append(sql);
			pageSql.append(" ) temp where rownum <= ").append(
					rowBounds.getLimit() + rowBounds.getOffset());
